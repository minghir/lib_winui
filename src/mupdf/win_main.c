...
int set_filename(const wchar_t* inputFilename) {
	if (!inputFilename) {
		pdfapp_error(&gapp, (char*)"filename?!");
		return 0;
	}

	//int code = WideCharToMultiByte(CP_UTF8, 0, inputFilename, -1, filename, sizeof(inputFilename), NULL, NULL);
	int code = WideCharToMultiByte(CP_UTF8, 0, inputFilename, -1, filename, PATH_MAX, NULL, NULL);

	if (code == 0) {
		pdfapp_error(&gapp, (char*)"cannot convert filename to utf-8");
		return 0;
	}
...
{
	MessageBoxA(hwndframe, msg, "PDF: Warning", MB_ICONWARNING);
}

void winerror(pdfapp_t *app, char *msg)
{
	MessageBoxA(hwndframe, msg, "PDF: Error", MB_ICONERROR);
	exit(1);
...
float GetDPIScaling(HWND hwnd) {
	HMONITOR hMonitor = MonitorFromWindow(hwnd, MONITOR_DEFAULTTONEAREST);
	UINT dpiX, dpiY;

	if (GetDpiForMonitor(hMonitor, MDT_EFFECTIVE_DPI, &dpiX, &dpiY) == S_OK) {
		float scale = dpiX / 96.0f; // 96 DPI = 100%
		wchar_t buffer[100];
		swprintf(buffer, 100, L"Scaling factor: %.0f%% (DPI: %u)", scale * 100, dpiX);
		//MessageBoxW(hwnd, buffer, L"Display Scaling", MB_OK | MB_ICONINFORMATION);
		return scale * 100;
	}
	else {
		MessageBoxW(hwnd, L"Nu am putut obține DPI-ul monitorului.", L"Eroare", MB_ICONERROR);
		return -1;
	}
}
...
void winprint(pdfapp_t* app)
{
	...
	if (!hdcPrinter) {
		MessageBox(hwndframe, L"Failed to get printer DC!", L"Error", MB_ICONERROR);
		return;
	}

	DOCINFO di = { 0 };
	di.cbSize = sizeof(DOCINFO);
	wchar_t wTitle[PATH_MAX];
	MultiByteToWideChar(CP_UTF8, 0, app->doctitle, -1, wTitle, PATH_MAX);
	di.lpszDocName = wTitle;

	if (StartDoc(hdcPrinter, &di) <= 0) {
		MessageBox(hwndframe, L"Failed to start document!", L"Error", MB_ICONERROR);
		DeleteDC(hdcPrinter);
		return;
	}
	...
	float dpiScale = (float)min_dpi / 72.0f;
	fz_colorspace* rgb_cs = fz_device_rgb(app->ctx);
	...
	for (int i = 0; i < app->pagecount; i++) {
		...
		fz_pixmap* pix = fz_new_pixmap_from_page_number(app->ctx, app->doc, i, ctm, rgb_cs, 0);
		...
		// Octeți necesari pe o linie (fără padding)
		int bytes_per_pixel = 3; // RGB 24-bit
		int mu_bytes_per_line = pix->w * bytes_per_pixel;

		// Stride-ul GDI (numărul de octeți pe linie, aliniat la cel mai apropiat multiplu de 4)
		int gdi_aligned_stride = (mu_bytes_per_line + 3) & ~3;
		int padding_bytes = gdi_aligned_stride - mu_bytes_per_line;

		// Dimensiunea totală a noului buffer
		size_t total_buffer_size = (size_t)gdi_aligned_stride * pix->h;

		// Alocăm memoria pentru buffer-ul aliniat GDI
		unsigned char* gdi_buffer = (unsigned char*)malloc(total_buffer_size);

		if (!gdi_buffer) {
			MessageBox(hwndframe, L"Eroare de alocare memorie GDI buffer!", L"Eroare", MB_ICONERROR);
			fz_drop_pixmap(app->ctx, pix);
			EndPage(hdcPrinter);
			break;
		}

		// --- 2. COPIERE MANUALĂ CU INTRODUCEREA PADDING-ULUI ---
		unsigned char* mu_src = pix->samples;
		unsigned char* gdi_dest = gdi_buffer;
		int mu_stride = pix->stride; // Stride-ul real folosit de MuPDF

		for (int y = 0; y < pix->h; y++) {
			// Copiem rândul de date RGB din MuPDF
			memcpy(gdi_dest, mu_src, mu_bytes_per_line);

			// Adăugăm padding (0) la finalul rândului GDI
			if (padding_bytes > 0) {
		...
		// --- 3. GESTIUNE BITMAPINFO (folosim noul buffer) ---
		BITMAPINFO bmi = { 0 };
		...
		// --- LOGICĂ SCALARE PROPORȚIONALĂ ȘI APEL STRETCHDIBITS ---
		int image_w = pix->w;
		int image_h = pix->h;
		int print_area_width = GetDeviceCaps(hdcPrinter, HORZRES);
		int print_area_height = GetDeviceCaps(hdcPrinter, VERTRES);

		float scale_w = (float)print_area_width / image_w;
		float scale_h = (float)print_area_height / image_h;
		float scale = (scale_w < scale_h) ? scale_w : scale_h;

		int final_w = (int)roundf(image_w * scale);
		int final_h = (int)roundf(image_h * scale);
		...
		int result = StretchDIBits(hdcPrinter,
			...
			0, 0, image_w, image_h,
			gdi_buffer, // FOLOSIM NOUL BUFFER ALINIAT
			&bmi,
			DIB_RGB_COLORS,
			SRCCOPY);

		// Curățenie
		free(gdi_buffer); // Eliberăm buffer-ul manual
		fz_drop_pixmap(app->ctx, pix);

		if (result == GDI_ERROR) {
			MessageBox(hwndframe, L"StretchDIBits a returnat GDI_ERROR!", L"Eroare GDI", MB_ICONERROR);
		}
...
{
	switch(MessageBoxA(hwndframe, "File has unsaved changes. Do you want to save", "PDF", MB_YESNOCANCEL))
	{
...
void wincopyfile(pdfapp_t *app, char *source, char *target)
{
	...
	if (CopyFile(wsource, wtarget, FALSE)) {
		wchar_t prefix[] = L"Fisierul a fost salvat: "; // Prefix

		wchar_t result[PATH_MAX];
		swprintf(result, PATH_MAX, L"%ls%ls", prefix, wtarget);
		MessageBoxW(hwndframe, result, L"Info", MB_OK);
	}
	else {
		DWORD error = GetLastError();
		MessageBoxW(hwndframe, L"Eroare", L"Info", MB_OK);
	}
}
...
void winremovefile(char* source){
	...
	if (sz == 0)
	{
		MessageBoxW(hwndframe, L"cannot convert filename to Unicode", L"Info", MB_OK);
		return;
	}
	if (DeleteFile(wsource)) {

	}
	else {
		MessageBoxW(hwndframe, L"cannot delete filename!!!!", L"Info", MB_OK);
	}
}
...
dloginfoproc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	//pdfapp_error(&gapp, "cannot create info dialog 11111");
	switch(message)
	{
	case WM_INITDIALOG:
 ...
 void winopen(HWND win_parent)
{
	...
	RECT r;
	ATOM a;
	pdfwinparent = win_parent;
	WNDCLASSW wcTest;
	if (GetClassInfoW(GetModuleHandle(NULL), L"FrameWindow", &wcTest)) {
		//MessageBoxW(NULL, L"Clasa este deja înregistrată!", L"Info", MB_OK);
	}
	else {
		...
		wc.hIcon = LoadIconA(wc.hInstance, "IDR_MAINFRAME");
		wc.hCursor = NULL; //LoadCursor(NULL, IDC_ARROW);
		wc.hbrBackground = NULL;
		wc.lpszMenuName = NULL;
		wc.lpszClassName = L"FrameWindow";
		//MessageBox(NULL, L"Eroare detectată1!", L"Debug Error", MB_ICONERROR | MB_OK);
		a = RegisterClassW(&wc);
		//MessageBox(NULL, L"Eroare detectată2!", L"Debug Error", MB_ICONERROR | MB_OK);
		assert(a);
		if (!a)
			pdfapp_error(&gapp, "cannot register frame window class");

		//MessageBox(NULL, L"Eroare detectată3!", L"Debug Error", MB_ICONERROR | MB_OK);
	...
	300, // initial x size
	300, // initial y size
	win_parent, // parent window handle
...
//	hdc = NULL;
	...
	SetWindowTextW(hwndframe, L"PDF");


	menu = GetSystemMenu(hwndframe, 0);
	AppendMenuW(menu, MF_SEPARATOR, 0, NULL);
	//AppendMenuW(menu, MF_STRING, ID_ABOUT, L"About FSNA PDF...");
	AppendMenuW(menu, MF_STRING, ID_DOCINFO, L"Propertietăţi document...");

	HMENU hMenu;
	hMenu = CreateMenu(); // Crearea meniului principal
	AppendMenuW(hMenu, MF_STRING, ID_BTN_PRINT, L"Tipareşte");
	AppendMenuW(hMenu, MF_STRING, ID_BTN_SAVE, L"Salvează PDF");
	AppendMenuW(hMenu, MF_STRING, ID_BTN_EMAIL, L"Trimite e-mail");
	AppendMenuW(hMenu, MF_STRING, ID_BTN_EXIT, L"Ieşire");
...
frameproc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		switch (LOWORD(wParam)) {
			case ID_BTN_PRINT:
				//MessageBoxW(hwnd, L"Print clicked!", L"Info", MB_OK);
				winprint(&gapp);
				return 0;
			...
			case ID_BTN_EXIT:
				...
				return 0;
			case ID_BTN_SAVE: {
				size_t leng = snprintf(NULL, 0, "%s%s", "Pdf\\", gapp.doctitle) + 1;
				char* dest = (char*)malloc(leng);
				if (dest == NULL) {
					return 0;
				}
				sprintf(dest, "%s%s", "Pdf\\", gapp.doctitle);
				wincopyfile(&gapp, gapp.docpath, dest);
				free(dest);
				return 0;
			}
		}
 ...
 void usage(const char *argv0)
{
 ...
 /*
 int winemail() {
//	 MessageBoxA(NULL, "AICICICI", "Aici!!!!", MB_OK);
	 HWND pWnd =  get_email_win();
	 if (pWnd && IsWindowVisible(pWnd)) {
		 MessageBoxA(NULL, "Fereastra deja deschisa!", "Warning:", MB_OK);
		 return 0;
	 }
	 //pdfwinparent
	 //hwndframe
	
	// file_path_send[PATH_MAX];
	 
	 show_email_dialog(hwndframe, &gapp); // Apelăm dialogul secundar
