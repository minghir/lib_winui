...
vShellEngine::vShellEngine() : m_running(true) {
    ...
    /*
    dbfConn = std::make_unique<dbfConnection>("DBF_NATIVE", dbfPath);
    if (dbfConn->openDatabase()) {
        LOG_SUCCESS(L"DBF Database opened in:" + dbfPath);
    }
    else {
        LOG_FATAL(L"DBF Database FAILD to opened in:" + dbfPath);
    }
    */
    
    dbfConn = std::make_unique<csvConnection>("csv", csvPath);
    
    if (dbfConn->openDatabase()) {
        LOG_SUCCESS(L"CSV Database opened in:" + csvPath);
    }
    else {
        LOG_FATAL(L"CSV Database FAILD to opened in:" + csvPath);
    }
