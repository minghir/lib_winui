...
int vGrid::addRow(const std::vector<std::wstring>& items) {
    ...
    if (itemIndex != -1) {
        for (size_t i = 1; i < items.size(); ++i) {
            ...
            ListView_SetItemText(m_handle, itemIndex, static_cast<int>(i), const_cast<LPWSTR>(text.c_str()));
        }
...
void vGrid::autoFitColumn(int columnIndex) {
    ...
    SIZE s;
    GetTextExtentPoint32(hdc, headerText, (int)wcslen(headerText), &s);
    ...
    for (int i = 0; i < rowCount; ++i) {
        ...
        ListView_GetItemText(m_handle, i, columnIndex, cellText, 1024);

        GetTextExtentPoint32(hdc, cellText, (int)wcslen(cellText), &s);
        if (s.cx + 15 > maxW) {
            maxW = s.cx + 15;
        }
    }
