...
void CodeLexer::loadSyntaxes(const std::string& folderPath) {
    ...
    try {
        // 1. Verificăm dacă folderul există cu adevărat
        if (!fs::exists(folderPath) || !fs::is_directory(folderPath)) {
            // LOG_ERROR(L"[CodeLexer] Folderul de sintaxe nu a fost găsit: " + str_to_wstr(folderPath));
            return;
        }

        // 2. Scanăm fiecare fișier din folder
        // Folosim un iterator explicit pentru a evita erori de tip C3536 pe unele versiuni de VS
        ...
        for (const auto& entry : it) {
            ...
            if (entry.is_regular_file() && entry.path().extension() == ".xml") {
                ...
                // Încărcăm definiția de limbaj folosind metoda loadLanguageFile pe care o avem deja
