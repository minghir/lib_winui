...
public:
    void applyLayout(vContainer& container) override {
        ...
        // --- PASUL 1: Analiză Resurse (Calculăm spațiul ocupat de elementele vizibile non-FILL) ---
        int occupiedWidth = 0;
        int fillCount = 0;

        for (auto const& [name, childPtr] : children) {
            // ADĂUGARE: Dacă controlul este ascuns logic, îl ignorăm complet în calcule
            if (!childPtr->isLogicVisible()) {
                continue;
            }
        ...
        int currentX = 0;
        for (auto const& [name, childPtr] : children) {
            // ADĂUGARE: Dacă este ascuns, sărim peste poziționare (nu avansăm nici cursorul currentX)
            if (!childPtr->isLogicVisible()) {
                continue;
            }
            ...
            // Avansăm cursorul doar cu spațiul consumat de elementele vizibile
            currentX += nextStepX;
        }
    }
};